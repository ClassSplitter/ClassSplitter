...
class Baseline1(GroupInterface):
    # 2014
    ...
    def original_graph_to_chain(self):
        # get quartiles q1, q2, q3
        weights = self.weight[:]
        weights.sort()
        n = len(weights)
        if n % 2 == 0:
            q2 = (weights[n // 2 - 1] + weights[n // 2]) / 2
        else:
            q2 = weights[n // 2]
        if (n // 2) % 2 == 0:
            q1 = (weights[n // 4 - 1] + weights[n // 4]) / 2
        else:
            q1 = weights[n // 4]
        if (n // 2) % 2 == 0:
            q3 = (weights[n * 3 // 4 - 1] + weights[n * 3 // 4]) / 2
        else:
            q3 = weights[n * 3 // 4]
